...
export const siteText = {
  ru: {
    ...
    stats: { eyebrow: "О бизнес-центре", heading1: "Триллиант — офисное здание класса А", heading2: "в центре г. Ташкента", paragraphs: ["Бизнес-центр Trilliant предлагает готовые офисы в аренду от собственника в центре Ташкента.", "Аренда офиса осуществляется без комиссии для арендатора. Помещения полностью готовы к заезду: выполнен качественный ремонт, предусмотрена мебель и инфраструктура для комфортной работы команды.", "Комплекс введён в эксплуатацию, полностью функционирует и соответствует уровню бизнес-центра класса А. В Trilliant доступны офисные помещения различной площади: от готовых блоков для небольших команд до просторных офисов для компаний, которым важно разместить сотрудников в одном современном деловом пространстве.", "Trilliant подходит компаниям, которые ищут офис в аренду в Ташкенте с возможностью быстрого въезда."], address: "адрес бизнес-центра", buildingClass: "класс здания", areaRange: "диапазон доступной площади для аренды", commissioned: "Введено в эксплуатацию", readiness: "готовность использования" },
    ...
    infrastructure: { eyebrow: "Инфраструктура", description: "В составе комплекса доступны гостиничные, ресторанные и wellness-сервисы." },
  ...
  uz: {
    ...
    infrastructure: { eyebrow: "Infratuzilma", description: "Majmua tarkibida mehmonxona, restoran va wellness xizmatlari mavjud." },
  ...
  en: {
    ...
    infrastructure: { eyebrow: "Amenities", description: "The complex provides hotel, restaurant and wellness services." },
