...
var locks=new Set(["__automatic_translation_v6"]);
...
window.trillAdminApplyOffice=function(formNode,office){for(var name of allNames)setControlValue(formNode.elements.namedItem(name),office[name],name);locks=new Set(["__automatic_translation_v6"]);writeLocks();};
...
var requestTranslations=async function(target,texts){
  var lastError=null;
  for(var attempt=0;attempt<3;attempt++){
    var response=await fetch("/api/admin/translate",{method:"POST",credentials:"same-origin",headers:{"content-type":"application/json"},body:JSON.stringify({target:target,texts:texts})});
    var result=await response.json().catch(function(){return {};});
    if(response.ok)return result.translations||{};
    lastError=Error(result.error||("Ошибка перевода: "+response.status));
    lastError.status=response.status;
    if(response.status!==429||attempt===2)throw lastError;
    var retryAfter=Number(response.headers.get("retry-after")||0);
    await wait(retryAfter>0?Math.min(retryAfter*1000,4000):(800*(attempt+1)));
  }
  throw lastError||Error("Ошибка перевода");
};
window.trillAdminFillTranslations=async function(formNode,values){
  var savedLocks=new Set(["__automatic_translation_v6"]);
  var rateLimited=false;
  for(var target of [["uz","узбекский"],["en","английский"]]){
    var language=target[0],source={};
    for(var base of bases){
      var sourceName=base,targetName=fieldName(base,language);
      var value=values[sourceName];
      if(Array.isArray(value)?value.length:String(value||"").trim())source[targetName]=value;
      else values[targetName]=Array.isArray(value)?[]:"";
    }
    var statusNode=formNode.querySelector("[data-admin-status]");
    if(statusNode){statusNode.textContent="Переводим все поля на "+target[1]+" язык...";statusNode.dataset.state="saving";}
    var translated;
    try{
      translated=await requestTranslations(language,source);
    }catch(error){
      var message=error instanceof Error?error.message:String(error||"");
      if((error&&error.status===429)||message.indexOf("429")>=0){
        rateLimited=true;
        if(statusNode){statusNode.textContent="Лимит сервиса перевода. Сохраняем объект с текущими переводами...";statusNode.dataset.state="saving";}
        continue;
      }
      throw error;
    }
    for(var key of Object.keys(source)){
      if(!Object.prototype.hasOwnProperty.call(translated,key))throw Error("Не переведено поле "+key);
      var sourceValue=source[key],translatedValue=translated[key];
      if(Array.isArray(sourceValue)){
        if(!Array.isArray(translatedValue)||translatedValue.length!==sourceValue.length)throw Error("Не полностью переведено поле "+key);
      }else if(String(sourceValue||"").trim()&&!String(translatedValue||"").trim())throw Error("Получен пустой перевод поля "+key);
      values[key]=translatedValue;
      var control=formNode.elements.namedItem(key);
      if(control&&"value" in control)control.value=Array.isArray(translatedValue)?translatedValue.join(String.fromCharCode(10)):String(translatedValue||"");
    }
  }
  values.translationLocks=Array.from(savedLocks);
  locks=savedLocks;
  writeLocks();
